...
user1.set_gender("Woman")
user1.set_age("25")
active_users[user1.id] = user1

user2 = User.get("user2")
user2.set_gender("Woman")
user2.set_age("29")
active_users[user2.id] = user2
        ...
        if data['image'] is not None:
            ...
            if user.userClassification(image):
                 if login_user(user):
                    ...
                    print("user logged in", current_user.id, "age", user.age, "gender", user.gender)
                    ...
                    print(user_json)
                    emit('login-status', {'success': True,'user': user_json })
            else:
                emit('login-status', {'success': False})
                logout_user()
        else:
